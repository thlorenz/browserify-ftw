...
function findUseStrictRange(astInnerBody, headEnd) {
  return astInnerBody
    .filter(function (e) {
      return e.type === 'ExpressionStatement' && e.expression.value === 'use strict';
    })
    .map(function (e) {
      return e.expression.range;
    })
    .map(function (e) {
      return { start: e[0] - headEnd, end: e[1] - headEnd }; // Get position in file, excluding the header
    })
    .shift();
}

function getWrapper(ast, getReturn, moveStrictWanted) {
  ...
  */
  ...
  var wrappers = ast.body
    .map(function (node) {
      ...
      var headEnd, tailStart, innerHeaderStart, innerHeaderEnd, useStrictRange;
      ...
      if (fn && fn.body) {
        ...
        useStrictRange = moveStrictWanted ? findUseStrictRange(fnInnerBody, headEnd) : undefined;
      } else {
        /* E.g.:
      ...
      return { 
        ...
        , hasUseStrict: useStrictRange ? Object.keys(useStrictRange).length > 0 : false
        , useStrictRange: useStrictRange
        , return: returnStmt
        , body: fnInnerBody
